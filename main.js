...
app.post('/register',
    ...
    async(req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            return res.status(400).json({ error: `Your ${errorArr.param} is wrong. Try Again` });
        }
...
app.get('/balance', setAuth, async (req, res) => {
    ...
    for (const asset of assets) {
        if(asset.balance !== 0)
            assetsList[asset.name] = asset.balance;
    }
...
app.post('/coins/:coinName/buy', setAuth, getPrice, async(req, res) => {
    ...
    try{
        ...
        if(err === 'Error_null') {
            ...
            return res.send({error : 'The quantity should be up to 4 decimal points'})
        }
    }
